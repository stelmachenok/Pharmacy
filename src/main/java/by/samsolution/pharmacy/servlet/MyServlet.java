...
//
//
////        OldService oldService = (OldService) getServletContext().getAttribute("OldService");
...
//        vc.put("medicaments", oldService.getAllMedicaments()); // атрибут "name" связывается с именем переменной $foo в шаблоне и помещается в контекст
//
...
////        oldService.getAllMedicaments().forEach(m -> {
////
////            pw.write(m.toString());
