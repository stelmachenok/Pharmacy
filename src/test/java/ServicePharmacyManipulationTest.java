...
public class ServicePharmacyManipulationTest {
    ...
    void addOnePharmacy() throws EntityAlreadyExistException, ObjectValidationFailedException {
        service.addPharmacy(new PharmacyDto("Доктор До Аптека", "улица Филимонова 3/2, Минск", "Табина", "8 017 210-07-72", "tabina", "qwerty", "1"));
    }

    @Test
    void addTwoDiffPharmacy() throws EntityAlreadyExistException, ObjectValidationFailedException {
        service.addPharmacy(new PharmacyDto("Доктор До Аптека", "улица Филимонова 3/2, Минск", "Табина", "8 017 210-07-72", "tabina", "qwerty", "1"));
        service.addPharmacy(new PharmacyDto("Аптека № 17", "улица Филимонова 119, Минск", "Атрошенко", "8 017 210-07-72", "atroshenko", "12345", "2"));
    }
    ...
    void addIncorrectNumberPharmacy() throws EntityAlreadyExistException, ObjectValidationFailedException {
        assertThrows(ObjectValidationFailedException.class, ()->{
            service.addPharmacy(new PharmacyDto("Доктор До Аптека", "улица Филимонова 3/2, Минск", "Табина", "eight zero one seven 210-07-72", "tabina", "qwerty", "1"));
        });
    }
